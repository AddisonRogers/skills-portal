...
async function main() {
  ...
  try {
    const roadmapContent = await readdir(
      `/home/dotracc/Downloads/developer-roadmap/src/data/roadmaps/ai-agents/content/`,
    );
    ...
    for (const node of roadmapContent) {
      ...
      handleSkill(
        `/home/dotracc/Downloads/developer-roadmap/src/data/roadmaps/ai-agents/content/${node}`,
      );
    }
  } catch (error) {
    console.error(error);
...
async function listAllResources() {
  try {
    const roadmapContent = await readdir(
      `./developer-roadmap/src/data/roadmaps/ai-agents/content/`,
    );
    ...
    for (const node of roadmapContent) {
      console.log(`Processing: ${node}`);
      const nodeContent = Bun.file(
        `./developer-roadmap/src/data/roadmaps/ai-agents/content/${node}`,
      );
    ...
    const sorted = Object.entries(linkCounts).sort(([, a], [, b]) => b - a);
...
async function listAllContentTypes() {
  try {
    ...
    for (const roadmap of roadmaps) {
      const roadmapContent = await readdir(
        `./developer-roadmap/src/data/roadmaps/${roadmap}/content/`,
      );

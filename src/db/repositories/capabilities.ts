...
export async function getCapabilitiesForUser(userId: string) {
  return db
    ...
    .innerJoin(
      capabilityUser,
      eq(capabilities.id, capabilityUser.capabilityId)
    )
    .where(eq(capabilityUser.userId, userId));
}

// Attach a capability to a user
export async function addCapabilityForUser(userId: string, capabilityId: number) {
...
export async function removeCapabilityForUser(userId: string, capabilityId: number) {
  return db
    .delete(capabilityUser)
    .where(and(eq(capabilityUser.userId, userId), eq(capabilityUser.capabilityId, capabilityId)));
}

// Add a new capability
export async function addCapability(capabilityName: string, capabilityDescription: string) {
...
export async function updateCapability(capabilityName: string, capabilityDescription: string) {
  return db
    .update(capabilities)
    .set({
      description: capabilityDescription,
...
export async function addRoadmapForCapability(capabilityId: number, roadmapId: string) {
...
export async function removeRoadmapForCapability(capabilityId: number, roadmapId: string) {
  return db
...
export async function getUsersForCapability(capabilityId: number) {
  return db
    ...
    .from(user)
    .innerJoin(
      capabilityUser, eq(user.id, capabilityUser.userId)
    )

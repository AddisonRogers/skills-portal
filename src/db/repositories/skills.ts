...
export async function getSkillsForRoadmapForUser(
  ...
  const data: PGSkillDataUser[] = await db
    ...
    .innerJoin(skillRoadmap, eq(skill.id, skillRoadmap.skillId))
    .innerJoin(roadmap, eq(skillRoadmap.roadmapId, roadmap.id))
    .leftJoin(userSkill, and(eq(skill.id, userSkill.skillId), eq(userSkill.userId, userId)))
    .where(eq(roadmap.id, roadmapId));
...
export async function getSkill(skillIdentifier: string) {
  try {
    ...
    const matchingSkill = allSkills.find((s) => skillNameToSlug(s.name) === skillIdentifier);
...
export async function getRoadmapSkills(userId: string | null): Promise<getRoadmapSkillsReturnType> {
  if (userId) {
    return db
      .select({
        id: skill.id,
...
export async function getSkillForUserEmail(
  ...
  try {
    ...
    const skillData = await db
      ...
      .from(skill)
      .where(and(eq(skill.machineName, machineNameSkill)));
    ...
    const userSkillData = await db
      ...
      .from(userSkill)
      .where(and(eq(userSkill.userId, userId), eq(userSkill.skillId, skillData[0].id)))
      .limit(1);
...
export async function setSkillProgressionForUserEmail(
  ...
  console.debug(`line 234 setting skill ${skill.id} for user ${user[0].id} to ${level}`);

...
export async function addFavouriteRoadmapEmail(
	userEmail: string,
	roadmapId: string,
) {
...
export async function removeFavouriteRoadmapId(
	userId: string,
	roadmapId: string,
) {
	return db
...
export async function removeFavouriteRoadmapEmail(
	userEmail: string,
	roadmapId: string,
) {
	const user = await getUserByEmail(userEmail);
...
export async function removeFavouriteSkillEmail(
	userEmail: string,
	skillId: number,
) {
	const user = await getUserByEmail(userEmail);
...
export async function addFavouriteSkillEmail(
	userEmail: string,
	skillId: number,
) {
	const user = await getUserByEmail(userEmail);

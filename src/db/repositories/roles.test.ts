...
describe("roles tests", () => {
    ...
    describe("addRoleToSomeone / removeRoleFromSomeone", () => {
        it("can add a role to user and make checks pass and then remove it", async () => {
            ...
            expect(await rolesModule.userHasRole(otherUser.email, "editor")).toBe(false);

            // Add role and check
            await rolesModule.addRoleToSomeone(otherUser.email, "editor");
            expect(await rolesModule.userHasRole(otherUser.email, "editor")).toBe(true);
    ...
    describe("addRole / removeRole", () => {
        ...
        it("can add and remove role by name", async () => {
            ...
            expect(await rolesModule.userHasRole(mockUser.email, tempRoleName)).toBe(true);

            // Remove role from user
            await rolesModule.removeRoleFromSomeone(mockUser.email, tempRoleName);
            expect(await rolesModule.userHasRole(mockUser.email, tempRoleName)).toBe(false);

            // Now remove role from system
            await rolesModule.removeRole(tempRoleName);

            // Should not be possible to assign anymore
            await expect(rolesModule.addRoleToSomeone(mockUser.email, tempRoleName)).rejects.toThrow();
        });
    });
    ...
    describe("updateRole", () => {
        ...
        it("updates a role's description", async () => {
            ...
            await expect(rolesModule.addRoleToSomeone(otherUser.email, newName)).resolves.not.toThrow();
        });
    });
});
...
export async function userHasRole(userEmail: string, roleName: string): Promise<boolean> {
...
export async function addRoleToSomeone(userEmail: string, roleName: string): Promise<void> {
  ...
  const foundRole = await db.select({
    id: role.id
  })
    .from(role)
...
export async function removeRoleFromSomeone(userEmail: string, roleName: string): Promise<void> {
  const foundUser = await db.select({
    id: user.id
  })
    .from(user)
  ...
  const result = await db.delete(user_roles)
    .where(and(
...
// TODO fix
export async function updateRole(roleName: string, roleDescription: string): Promise<void> {
  const result = await db
    .update(role)
    .set({
      name: roleName,

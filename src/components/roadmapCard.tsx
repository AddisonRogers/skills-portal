    ...
    return (
            ...
            <div className="flex justify-between items-start mb-2">
                ...
                <div className="flex space-x-2">
                    {roadmap.suggestedBy === 'manager' && (
                        <Briefcase size={18} className="text-fsp-core-teal" title="Suggested by manager"/>
                    )}
                    {roadmap.suggestedBy === 'role' && (
                        <User size={18} className="text-fsp-core-teal" title="Suggested by role"/>
                    )}
                    {roadmap.isFavorite && (
                        <Star size={18} className="text-yellow-400 fill-yellow-400" title="Favorited"/>
                    )}
                </div>
            </div>
            <div className="mb-2">
        <span className="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700">
          {roadmap.capability}
            ...
            {roadmap.usersLearning} {roadmap.usersLearning === 1 ? 'person' : 'people'} learning
          </span>
                </div>
            )}
        </div>)

...
export async function calculateTimeRemaining(personName: string): Promise<number> {
  try {
    ...
    if (Array.isArray(kardsData)) {
      for (const kard of kardsData) {
        ...
        if (kard.effort && kard.percentComplete) {
          const remainingEffort = kard.effort * (1 - (kard.percentComplete / 100));
...
export async function calculateUserXP(userId: string): Promise<number> {
  try {
    ...
    const totalXP = userSkills.reduce((sum, item) => sum + (item.xpAmount || 0), 0);
...
export async function searchPeopleBySkill(skillName: string): Promise<PersonWithAccount[]> {
  try {

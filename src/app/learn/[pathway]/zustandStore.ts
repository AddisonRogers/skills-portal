...
export const useFlowStore = create<FlowState>((set, get) => ({
		...
		set((state) => {
			...
			changes.forEach((change) => {
				if (change.type === "position") {
					...
					const originalNode = state.originalNodes.find(
						(n) => n.id === change.id,
					);
					...
					if (originalNode && currentNode) {
						const positionChanged =
							Math.abs(
								originalNode.position.x - change.position?.x ||
									currentNode.position.x,
							) > 1 ||
							Math.abs(
								originalNode.position.y - change.position?.y ||
									currentNode.position.y,
							) > 1;

						if (positionChanged && change.dragging) {
							// Only track when actually dragging
	...
	getChangedEdges: () => {
		...
		console.log("changedEdgeIds:", changedEdgeIds);

...
export const useFlowStore = create<FlowState>((set, get) => ({
    ...
    set((state) => {
      ...
      changes.forEach(change => {
        if (change.type === 'position') {
          // Only track if this is a meaningful position change (user dragging)
          // Skip automatic layout changes
          const originalNode = state.originalNodes.find(n => n.id === change.id);
          const currentNode = state.nodes.find(n => n.id === change.id);

          if (originalNode && currentNode) {
            const positionChanged =
              Math.abs(originalNode.position.x - change.position?.x || currentNode.position.x) > 1 ||
              Math.abs(originalNode.position.y - change.position?.y || currentNode.position.y) > 1;

            if (positionChanged && change.dragging) { // Only track when actually dragging
        ...
        // Don't track 'select' or 'dimensions' changes
      });
  ...
  getChangedNodes: () => {
    ...
    console.log('changedNodeIds:', changedNodeIds);
    console.log('nodes count:', nodes.length);

    const filtered = nodes.filter((node) => changedNodeIds.has(node.id));

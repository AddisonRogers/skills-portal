    ...
    <div className="container mx-auto py-8">
      ...
      {roadmapsWithSkills.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-lg text-muted-foreground">No roadmaps available.</p>
        </div>
      ) : (
        <div className="grid gap-8">
          {Array.from(roadmapsWithSkills.entries()).map(([roadmapId, skills]) => (
            <Card key={roadmapId} className="overflow-hidden">
              <CardHeader className="bg-muted/50">
                <CardTitle>{skills[0]?.roadmapName || `Roadmap ${roadmapId}`}</CardTitle>
                {skills[0]?.roadmapDescription && (
                  <CardDescription>{skills[0].roadmapDescription}</CardDescription>
                )}
              </CardHeader>
              <CardContent className="pt-6">
                {skills.length === 0 ? (
                  <p className="text-muted-foreground text-sm">No skills available for this roadmap.</p>
                ) : (
                      ...
                      >
                        <Card className="h-full hover:shadow-md transition-shadow relative">
                          ...
                          <CardHeader className="pb-2">
                            <CardTitle className="text-lg pr-16">{skill.name}</CardTitle>
                            {skill.madeBy && (

...
export function SkillsGrid({ allSkills, roadmapsWithSkills, isLoggedIn }: SkillsGridProps) {
  const [skillName] = useQueryState('skillName', parseAsString.withDefault(''));
  const [roadmapId] = useQueryState('roadmapId', parseAsString.withDefault('all'));
  ...
  const filteredSkills = useMemo(() => {
    ...
    if (skillName.trim()) {
      skills = skills.filter((skill) => skill.name.toLowerCase().includes(skillName.toLowerCase()));
    }
      ...
      {filteredSkills.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredSkills.map((skill) => (
            <Link key={skill.id} href={`/skills/${skillNameToSlug(skill.name)}`} className="block">

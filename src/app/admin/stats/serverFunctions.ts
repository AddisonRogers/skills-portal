...
export type getPopularSkillsStatsReturnTypePromised = Promise<getPopularSkillsStatsReturnType>;
...
export async function getPopularSkillsStats(): getPopularSkillsStatsReturnTypePromised {
  const popularSkills = await db
    ...
    .from(userSkill)
    .innerJoin(skill, sql`${userSkill.skillId}
    =
    ${skill.id}`)
    .groupBy(skill.name)
...
export type getProficiencyDistributionStatsReturnTypePromised = Promise<getProficiencyDistributionStatsReturnType>
...
export type getSkillsOverTimeStatsReturnTypePromised = Promise<getSkillsOverTimeStatsReturnType>;
